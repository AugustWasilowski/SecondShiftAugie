    ...
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            ...
            if "verification_url" in response_data and "user_code" in response_data:
                ...
                print(
                    f"Enter the following code when prompted: {response_data['user_code']}"
                )
            else:
                raise KeyError(
                    "Unable to find 'verification_url' or 'user_code' in the response data."
                )
            ...
            while True:
                ...
                if token_response.status_code == 200:
                    creds = Credentials.from_authorized_user_info(
                        info=token_response_data, scopes=SCOPES
                    )
                    break
                elif token_response_data["error"] != "authorization_pending":
                    raise Exception(
                        f"Error occurred during authorization: {token_response_data['error']}"
                    )
